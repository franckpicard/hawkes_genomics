...
getparam2 <- function(data, K = 10) {
  res %>% 
    ...
    dplyr::mutate(
      vs_name = levels(as.factor(colnames(.)))[vs_name]
    ) %>% 
    tidyr::pivot_longer(cols = !c(vs_name)) %>% 
    dplyr::relocate(name) %>% 
    dplyr::arrange(name, vs_name) %>% 
    group_by(name, vs_name)
}
```
...
convolve_with_kernel <- function(data, width, K = 10) {
  getparam2(data, K = K) %>% 
  dplyr::group_by(name, vs_name) %>% 
  dplyr::mutate(
    name_width = purrr::map_dbl(name, function(x, width = width, names = names){
        width[which(names == x)]
      }, width = width, names = colnames(data)
    ),
    vs_name_width = purrr::map_dbl(vs_name, function(x, width = width, names = names){
        width[which(names == x)]
      }, width = width, names = colnames(data)
    )
  ) %>% 
  dplyr::mutate(
    convolution = purrr::pmap_dbl(
      list(value, name_width, vs_name_width), function(value, name_width, vs_name_width){
      value %>% 
        convolve(name_width) %>% 
        convolve(vs_name_width)
    })
  )
}
convolve_with_kernel(res, width = beds$interval_size, K = 10)
...
ww <- convolve_to_histogram(
  res = res,
  eta = beds$interval_size
)
