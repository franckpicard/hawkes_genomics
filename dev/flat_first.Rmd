...
preprocess_bed <- function(files, names = files, maxlag = 1e5, prefix = "clean_", outdir = "./") {
  tibble::tibble(
    ...
    dplyr::mutate(
      bed = purrr::map(files, function(path){
          vroom::vroom(
            path,
  ...
  dplyr::mutate(
    window = purrr::pmap_int(
      list(names, start, stop), function(names, start, stop) {
        min(start) %/% maxlag + 1
      })
  ) %>% 
  ...
  nest(data = c(chr, start, stop)) %>% 
  dplyr::mutate(
    path = purrr::map2(files, data, function(path, data) {
